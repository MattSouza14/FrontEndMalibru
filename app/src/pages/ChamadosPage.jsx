            ...
            {chamados.length === 0 ? (
              ...
              chamados.map((chamado) => (
                <Fragment key={chamado.id}>
                  <tr className="hover:bg-ws-canvas">
                    <td>
                      <p className="font-medium text-ws-bright">{chamado.assunto}</p>
                      <p className="text-xs text-ws-muted mt-1 line-clamp-1">{chamado.descricao}</p>
                    </td>
                    <td className="text-ws-secondary">{getFerramentaLabel(chamado.ferramentaRemota)}</td>
                    <td className="whitespace-nowrap">
                      <ChamadoStatusBadge status={chamado.status} />
                    </td>
                    <td className="text-ws-secondary whitespace-nowrap tabular-nums">
                    ...
                    <tr className="bg-ws-canvas">
                      <td colSpan={5} className="px-6 py-5">
                        {expandedLoadingId === chamado.id ? (
                          <p className="text-sm text-ws-muted">Carregando conversa...</p>
                        ) : (
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                          <div className="space-y-3 text-sm">
                            <p>
                              <span className="form-label block mb-1">Telefone</span>
                              <span className="text-ws-secondary">{chamado.telefoneContato}</span>
                            </p>
                            <p>
                              <span className="form-label block mb-1">Código remoto</span>
                              <span className="text-ws-secondary font-mono">{chamado.codigoAcessoRemoto}</span>
                            </p>
                            <p>
                              <span className="form-label block mb-1">E-mail</span>
                              <span className="text-ws-secondary">{chamado.email}</span>
                            </p>
                          </div>
